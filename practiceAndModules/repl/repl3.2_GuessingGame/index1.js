...
function play() {
    ...
    parseInt(uNum = prompt('Enter a number between 1 and 10'));

        //if guess is empty (click OK with input)
            while(uNum === '') {
                uNum = prompt('Uhmm...you forgot the number.  Please try again. Enter a number between 1 and 10');
                };
            //if guess less than min       
            while(uNum < min) {
                uNum = prompt(`The number must be greater than or equal to ${min}. Please try again.`);
                };  
             //if guess greater than max       
             while(uNum < min) {
                uNum = prompt(`The number must be less than or equal to ${max}. Please try again.`);
                };    

    
        // if count less than 3 and guess == random: alert win, return
        ...
        //if count == 3 and guess != random: alert lose, return
        //if count == 3 and guess == random: alert win, return
    if(aCount == 3) {
        if (uNum != rNum) {
